...
test('semantic affinity follows live modal energy rather than merely the nearest drive frequency',()=>{
  ...
  const pose={entityId:'heart',x:10,y:20,z:0,scale:1,extent:{width:400,height:400,rotation:0,normalized:true},tint:'#ffffff',tintWeight:0,forces:{mode:'none' as const,strength:0,radius:5,spin:0},sequence:{linkIndex:0,nextIndex:0,progress:0,phase:'hold' as const,step:0,linkCount:1}};
...
test('semantic colour radius follows force radius only when explicitly configured',()=>{
  const pose={entityId:'heart',x:0,y:0,z:0,scale:1,extent:{width:400,height:400,rotation:0,normalized:true},tint:'#ffffff',tintWeight:0,forces:{mode:'none' as const,strength:0,radius:5,spin:0},sequence:{linkIndex:0,nextIndex:0,progress:0,phase:'hold' as const,step:0,linkCount:1}};
