...
export class DocumentStore {
 ...
 finish(){if(!this.baseline)return;const before=this.baseline;this.baseline=null;if(JSON.stringify(before)!==JSON.stringify(this.document)){this.undoStack.push(before);if(this.undoStack.length>60)this.undoStack.shift();this.redoStack=[];this.document.updatedAt=new Date().toISOString();this.touch();}}
 change(fn:(doc:Journey)=>void){this.begin();try{fn(this.document);this.finish();}catch(error){if(this.baseline)this.document=this.baseline;this.baseline=null;this.touch();throw error;}}
