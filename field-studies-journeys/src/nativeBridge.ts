...
import {NATIVE_BINDINGS,WORLD_SCALE,baseValue,bindValue,nativeBinding,automationTarget,entityTargets} from './nativeParameters';
...
function shapeOf(e:Pick<Entity,'shape'|'text'|'yantraId'|'templateFrequency'>,native?:NativeShape):NativeShape{
 if(e.shape==='yantra')return {...native,kind:'yantra',yantraId:e.yantraId??native?.yantraId??'anahata'};
 if(e.shape==='cymatic')return {...native,kind:'cymatic',frequencyHz:e.templateFrequency??native?.frequencyHz??396};
...
export function toNativeEntity(e:Entity):NativeEntity{
 ...
 const sequence={...DEFAULT_SEQUENCE,...original?.sequence,...e.sequence,links:[]};
...
export function toNativeConfig(s:Scene):PointCloudConfig{
 ...
 const first=s.entities.find(e=>e.kind==='formation');cfg.sourceType='composition';cfg.customImage=first?.source?.kind==='image'?clone(first.source.image):undefined;cfg.asciiGlyph=first?.source?.kind==='ascii'?clone(first.source.ascii):undefined;
 ...
 cfg.cymatics={...DEFAULT_CYMATIC_MEDIUM,...cfg.cymatics,enabled:s.engine.resonanceEnabled,engine:s.engine.resonatorMode??'resonator',followFocus:s.composition.frequencyDriver==='focus',autoSweep:s.engine.autoSweep&&s.composition.frequencyDriver==='automation',sweep:{glideS:8,dwellS:2,...cfg.cymatics?.sweep,enabled:s.engine.autoSweep&&s.composition.frequencyDriver==='automation',direction:s.engine.sweepDirection}};
...
export function fromNativeEntity(e:NativeEntity):Entity{
 ...
 out.shape=shellShape(e.shape);out.yantraId=e.shape.yantraId;out.templateFrequency=e.shape.frequencyHz;
 ...
 out.sequence={...e.sequence,manual:e.sequence.advance==='off'&&e.sequence.links.length>1,enabled:e.sequence.advance!=='off',clock:e.sequence.advance==='morphCycle'?'morph':'seconds',steps:(e.sequence.links.length?e.sequence.links:[{id:e.id+'_base',shape:e.shape}]).map(k=>({id:k.id,native:clone(k),holdOverride:k.hold!==undefined,transitionOverride:k.transition!==undefined,text:k.shape.text??'',shape:shellShape(k.shape),yantraId:k.shape.yantraId,templateFrequency:k.shape.frequencyHz,hold:k.hold??e.sequence.hold,transition:k.transition??e.sequence.transition,
...
export function nativeSnapshotToJourney(raw:unknown,index=0):Journey{
 ...
 s.native={config:clone(cfg),original:clone(raw)};s.text=[];
 s.engine={...DEFAULT_ENGINE_SETTINGS,resonanceEnabled:cfg.cymatics?.enabled??false,morphEnabled:cfg.toroidalMorph?.enabled??false,autoOscillate:cfg.toroidalMorph?.autoOscillate??true,trajectory:cfg.toroidalMorph?.trajectory??'linear',driveShape:cfg.toroidalMorph?.driveShape??'sine',relationalEnabled:cfg.relational?.enabled??false,relationalMode:cfg.relational?.mode as any??'orbital',pointerMode:cfg.interaction.mode,colorMode:cfg.color?.mode??'monochrome',colorEnabled:cfg.color?.enabled??false,mediumPlane:cfg.composition?.plane??'vertical',autoSweep:cfg.cymatics?.autoSweep??false,sweepDirection:cfg.cymatics?.sweep?.direction??'ascent'};
 ...
 s.engine.paletteSource=cfg.color?.customPaletteColors?.length?'custom':'legacy';if(cfg.paperGrain===undefined)s.field.params.grain=0;s.engine.grainProfile=!!cfg.material;s.engine.backgroundMode=cfg.backgroundMode??cfg.color?.backgroundMode??'solid';s.engine.dotShape=cfg.dotShape??'circle';
 ...
 if(s.entities[0]&&!s.entities[0].source){if(cfg.customImage?.dataUrl)s.entities[0].source={kind:'image',image:clone(cfg.customImage)};else if(cfg.asciiGlyph?.text)s.entities[0].source={kind:'ascii',ascii:clone(cfg.asciiGlyph)};}
 ...
 s.composition.frequencyDriver=cfg.cymatics?.autoSweep||cfg.automations?.some(l=>l.enabled&&l.path==='cymatics.frequencyHz')?'automation':cfg.cymatics?.followFocus?'focus':'manual';
 ...
 s.automation=(cfg.automations??[]).map(l=>{
  ...
  return{id:l.id,nativeId:l.id,nativePath:l.path,entityId:eb?.entityId,enabled:l.enabled,target:b?'field.'+b.key:eb?.target??'native:'+encodeURIComponent(l.path),type:l.type==='lfo'?'lfo':'ramp',wave:l.waveform==='randomStep'?'steps':l.waveform==='smoothRandom'?'smooth':l.waveform??'sine',min:(l.type==='lfo'?l.min??0:l.from??0)/factor,max:(l.type==='lfo'?l.max??1:l.to??1)/factor,rate:l.rateHz??.25,phase:l.phase??0,blend:l.blend??'replace',duration:l.durationS??2,delay:l.delayS??0,loop:l.loop==='restart'?'loop':l.loop==='pingpong'?'pingpong':'once',firedAt:l.fireToken??null,easing:l.easing??'smooth'};
 });
 ...
 checkNativeLimits(s);return validateJourney(j);
}
