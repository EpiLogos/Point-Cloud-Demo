...
export function installPhysis(getSource:()=>DesktopScene,load:(source:DesktopScene)=>void,toast:(text:string)=>void){
 ...
 async function render(){
  ...
  panel.innerHTML=`<header><h3>Physis</h3><button class="icon-button" data-physis="close" aria-label="Close Physis desktop">×</button></header><p data-physis-status>${esc(statusText())}</p><div class="button-row"><button class="primary" data-physis="send">Use this scene on desktop</button><button class="secondary" data-physis="toggle">${state?.enabled?'Turn off':'Turn on'}</button></div><label class="control"><span>Opacity</span><input aria-label="Desktop opacity" type="range" min=".05" max="1" step=".05" value="${state?.opacity??.55}" data-physis-setting="opacity"></label><label class="control"><span>Quality</span><select aria-label="Desktop quality" data-physis-setting="quality"><option value="20" ${state?.fps===20?'selected':''}>Gentle · 20 fps</option><option value="30" ${state?.fps===30?'selected':''}>Balanced · 30 fps</option><option value="60" ${state?.fps===60?'selected':''}>Fluid · 60 fps</option></select></label><div class="button-row"><button class="secondary" data-physis="save">Save expression to disk</button></div><label class="control"><span>Saved expressions</span><select aria-label="Physis saved expressions" data-physis-setting="load"><option value="">Load from disk…</option>${scenes.filter((s:any)=>s.expression).map((s:any)=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('')}</select></label><p class="control-note">Image capture and Save recording write directly to your Physis library. Completed videos become screensaver scenes.</p><div class="physis-media">${media.slice(0,5).map((m:any)=>`<a href="${esc(m.url)}" target="_blank" rel="noreferrer">${m.kind==='images'?'PNG':'Video'} · ${esc(m.name)}</a>`).join('')}</div>`;
 }
 ...
 panel.addEventListener('change',e=>{const target=e.target as HTMLInputElement;void run(async()=>{
  ...
  if(target.dataset.physisSetting==='quality'){const fps=Number(target.value);await api('/api/overlay',{fps,particleLimit:fps===20?50000:fps===30?100000:200000,pixelRatio:fps===20?.75:1});}
