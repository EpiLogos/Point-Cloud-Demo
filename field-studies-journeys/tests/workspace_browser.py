...
try:
 with browser_session({'width':1440,'height':1000},'reduce') as (browser,ctx,p):
  ...
  boxes=p.locator('#tool-rail button').evaluate_all('(buttons)=>buttons.map(b=>{const r=b.getBoundingClientRect();const s=b.querySelector("svg").getBoundingClientRect();return {cx:r.x+r.width/2,sx:s.x+s.width/2,width:r.width};})')
  check('All seven icon centres align in a thinner rail; Interact comes first',len(boxes)==7 and max(x['cx'] for x in boxes)-min(x['cx'] for x in boxes)<.1 and all(abs(x['cx']-x['sx'])<.1 for x in boxes) and p.locator('#tool-rail').bounding_box()['width']<=46 and p.locator('#tool-rail button').first.get_attribute('data-rail')=='interact',boxes)
  before=gpu(p);base=doc(p)
  for tool in ['select','formation','text','pin','orbit','objects']:
   act(p,'objects' if tool=='objects' else 'tool-'+tool);a=state(p)
   act(p,'objects' if tool=='objects' else 'tool-'+tool);b=state(p)
   require(a['railExpanded'] and not b['railExpanded'] and not b['inspectorOpen'] and not p.locator('#shape-picker').is_visible() and not p.locator('#view-controls').is_visible(),tool+' close cycle')
   act(p,'objects' if tool=='objects' else 'tool-'+tool);c=state(p)
   require(c['tool']=='interact' and not c['editing'],tool+' third-click Interact')
  check('Every tool cycles open, closed, then back to Interact without mutating the expression',doc(p)==base and gpu(p)['seeds']==before['seeds'])
  act(p,'tool-select');act(p,'close-inspector');act(p,'tool-select')
  check('Closing a menu explicitly also makes the next same-tool click return to Interact',state(p)['tool']=='interact')
  ...
  act(p,'tool-orbit');check('Camera tool has no duplicate persistent toolbar or instruction pill',not p.locator('#view-controls').is_visible() and not p.locator('#tool-hint').is_visible() and p.locator('#orbit-control').is_visible());act(p,'tool-interact')
  ...
  check('Narrow layout keeps controller, capture and transport without horizontal overflow',p.locator('#orbit-control').is_visible() and p.locator('#play-button').is_visible() and p.locator('#record-button').is_visible() and p.evaluate('document.documentElement.scrollWidth<=innerWidth'))
