...
if str(idle) in changes:
    subprocess.run(['systemd-run','--user','--collect',
                    '--unit=physis-apply-shell',
                    '/usr/bin/python3',str(root/'scripts/apply-shell.py')],check=True)
    print('Idle service reload queued; waits until the desktop is unlocked.')
