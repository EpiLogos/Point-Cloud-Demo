...
static void monitor_added(GdkDisplay *display, GdkMonitor *monitor, gpointer unused) {
  ...
  input_region(window,NULL); input_region(view,NULL);
}
...
int main(int argc,char **argv) {
  ...
  if(!gtk_layer_is_supported()) { g_printerr("Wayland layer-shell is unavailable\n");return 1; }
  render_url=argv[1];
  windows=g_hash_table_new_full(g_direct_hash,g_direct_equal,g_object_unref,(GDestroyNotify)gtk_widget_destroy);
  GdkDisplay *display=gdk_display_get_default();
  for(int i=0;i<gdk_display_get_n_monitors(display);i++) monitor_added(display,gdk_display_get_monitor(display,i),NULL);
