...
function status() { return { ...state, running: !!renderer && rendererReady, starting: !!renderer && !rendererReady, suspended, error, scene: currentScene, library }; }
function broadcast() { const data = `data: ${JSON.stringify(status())}\n\n`; for (const res of clients) res.write(data); }
...
app.get('/api/health', (_req, res) => res.json({ ok: true, app: 'physis' }));
app.get('/api/state', (_req, res) => res.json(status()));
app.get('/api/events', (req, res) => {
  res.set({'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', Connection: 'keep-alive'}); res.flushHeaders(); clients.add(res); res.write(`data: ${JSON.stringify(status())}\n\n`);
...
app.post('/api/scenes', wrap(async (req, res) => {
  const {name, config, camera} = req.body || {};
  if (!config || typeof config !== 'object' || Array.isArray(config) || !config.glyph) return res.status(400).json({error:'A scene configuration with glyph is required'});
  const scene = {version:1, id:randomUUID(), name:String(name || 'Untitled scene').slice(0,120), createdAt:new Date().toISOString(), config, camera:camera || null};
...
app.post('/api/media/:kind', express.raw({type:['image/png','video/webm'],limit:'256mb'}), wrap(async (req,res) => {
  const kind=req.params.kind;
  const png=kind==='images' && req.is('image/png'), video=kind==='videos' && req.is('video/webm');
  if ((!png && !video) || !Buffer.isBuffer(req.body) || req.body.length < 8) return res.status(400).json({error:'Expected PNG image or WebM video'});
  if (png ? !req.body.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10])) : !req.body.subarray(0,4).equals(Buffer.from([26,69,223,163]))) return res.status(400).json({error:'Invalid media signature'});
  const scene=await readScene(req.query.sceneId);
  const id=randomUUID(), filename=`${id}.${png?'png':'webm'}`;
  const item={id,kind,filename,sceneId:scene.id,name:scene.name,createdAt:new Date().toISOString(),url:`/media/${kind}/${filename}`};
...
app.get(['/','/render'], (_req,res) => res.sendFile(path.join(root,'dist/index.html')));
...
for (const signal of ['SIGTERM','SIGINT']) process.on(signal,()=>{stopping=true;clearInterval(poll);if(renderer)renderer.kill('SIGTERM');for(const res of clients)res.end();server.close(()=>process.exit(0));setTimeout(()=>process.exit(0),2000).unref();});
